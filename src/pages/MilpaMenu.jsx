import { Link } from "react-router-dom";

export default function MilpaMenu() {
  const juegos = [
    {
      emoji: "🎴",
      titulo: "Adivina la Planta",
      maya: "Na'at le paak'al",
      descripcion: "Te damos pistas y tú adivinas qué planta de la milpa es.",
      descripcionMaya: "K ts'aik pistas yéetel ta na'atik ba'ax paak'al lelo'",
      ruta: "/game/milpa/adivina",
      fondo: "bg-gradient-to-br from-yellow-400 to-orange-500",
      color: "hover:bg-[#FFF9C4] border-yellow-300",
      btn: "bg-yellow-600 hover:bg-yellow-800",
      decorEmoji: ["🌽", "🌿", "⭐"],
    },
    {
      emoji: "🌽",
      titulo: "Construye tu Milpa",
      maya: "Pak' le kool",
      descripcion: "Arrastra y coloca las plantas correctas en su lugar dentro de la milpa.",
      descripcionMaya: "Jaats'uts u ts'aik le paak'alo'ob ich u kúuchil le kool",
      ruta: "/game/milpa/construye",
      fondo: "bg-gradient-to-br from-green-400 to-lime-600",
      color: "hover:bg-[#E8F5E9] border-green-300",
      btn: "bg-green-700 hover:bg-green-900",
      decorEmoji: ["🫘", "🎃", "🌶️"],
    },
  ];

  return (
    <div className="min-h-screen bg-[#FFF9E6]">

      {/* HERO */}
      <div className="relative bg-gradient-to-br from-[#854F0B] to-[#E9C46A] px-8 py-14 overflow-hidden">
        <div className="absolute top-4 right-8 text-[120px] opacity-10 animate-spin" style={{animationDuration:"20s"}}>🌽</div>
        <div className="absolute bottom-4 left-8 text-[80px] opacity-10">🌿</div>
        <div className="absolute top-20 left-1/3 text-[60px] opacity-10">⭐</div>
        <div className="absolute bottom-10 right-40 text-[70px] opacity-10">🌺</div>

        <Link to="/saberes" className="text-yellow-100 font-bold text-lg hover:text-white transition relative z-10">
          ← Volver
        </Link>

        <div className="relative z-10 mt-6 text-center">
          <div className="text-8xl mb-4 drop-shadow-2xl">🌽</div>
          <div className="inline-block bg-white/10 text-yellow-100 text-sm font-bold px-4 py-1 rounded-full mb-4">
            🌿 Saberes Ancestrales
          </div>
          <h1 className="text-5xl md:text-7xl font-bold text-white leading-tight mb-2">
            La Milpa Maya
          </h1>
          <p className="italic text-yellow-100 text-xl mb-4">Le Kool Maya</p>
          <p className="text-yellow-50 text-lg max-w-xl mx-auto">
            La milpa es el sistema de cultivo más importante de la cultura maya. ¡Aprende jugando!
          </p>
          <p className="italic text-yellow-200 text-sm mt-2">
            Le kool u yuum u paak'al maaya'ob — Ko'ox kanik báaxal
          </p>
        </div>
      </div>

      {/* OLA */}
      <div className="w-full overflow-hidden -mt-1">
        <svg viewBox="0 0 1440 80" xmlns="http://www.w3.org/2000/svg">
          <path fill="#854F0B" d="M0,40 C480,80 960,0 1440,40 L1440,0 L0,0 Z"/>
        </svg>
      </div>

      {/* TARJETAS */}
      <div className="px-8 pb-16 -mt-4">
        <p className="text-center text-yellow-800 font-bold text-xl mb-2">
          🎮 Elige tu juego
        </p>
        <p className="text-center italic text-[#A98467] text-sm mb-10">
          Ko'ox báaxal — ¡Vamos a jugar!
        </p>

        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {juegos.map((juego, index) => (
            <div
              key={index}
              className={`bg-white rounded-[30px] shadow-2xl border-2 overflow-hidden transition duration-300 hover:-translate-y-4 hover:shadow-3xl ${juego.color}`}
            >
              {/* Header colorido */}
              <div className={`${juego.fondo} p-8 relative overflow-hidden`}>
                {juego.decorEmoji.map((e, i) => (
                  <span
                    key={i}
                    className="absolute opacity-20 text-5xl"
                    style={{
                      top: i === 0 ? "8px" : i === 1 ? "40px" : "10px",
                      right: i === 0 ? "12px" : i === 1 ? "60px" : "90px",
                      fontSize: i === 0 ? "60px" : i === 1 ? "40px" : "50px",
                    }}
                  >
                    {e}
                  </span>
                ))}
                <div className="text-8xl mb-2 drop-shadow-lg relative z-10 text-center">
                  {juego.emoji}
                </div>
              </div>

              {/* Contenido */}
              <div className="p-8">
                <h2 className="text-2xl font-bold text-gray-800 mb-1">{juego.titulo}</h2>
                <p className="italic text-[#A98467] text-sm mb-4">{juego.maya}</p>
                <p className="text-gray-600 leading-relaxed mb-1 text-sm">{juego.descripcion}</p>
                <p className="text-xs italic text-[#A98467] leading-relaxed mb-6">{juego.descripcionMaya}</p>
                <Link
                  to={juego.ruta}
                  className={`${juego.btn} text-white px-6 py-3 rounded-2xl font-bold hover:scale-105 transition duration-300 w-full text-center block text-lg shadow-lg`}
                >
                  ¡Jugar ahora! 🎮
                </Link>
              </div>
            </div>
          ))}
        </div>

        {/* Banner informativo */}
        <div className="mt-12 max-w-4xl mx-auto bg-gradient-to-r from-[#854F0B] to-[#E9C46A] rounded-[30px] p-8 text-center shadow-2xl relative overflow-hidden">
          <div className="absolute top-2 left-4 text-6xl opacity-10">🌺</div>
          <div className="absolute bottom-2 right-4 text-6xl opacity-10">🫘</div>
          <p className="text-5xl mb-3">🌽</p>
          <h3 className="text-3xl font-bold text-white mb-2">¿Sabías que...?</h3>
          <p className="italic text-yellow-100 mb-3">Ba'ax a wojel wáaj...</p>
          <p className="text-yellow-50 max-w-xl mx-auto">
            La milpa maya combina maíz, frijol, calabaza y chile — las cuatro plantas sagradas que han alimentado al pueblo maya por más de 3,000 años.
          </p>
          <p className="italic text-yellow-200 text-sm mt-3">
            Le kool maya ku ts'aik ixi'im, iib, k'uum yéetel iik — Le paak'alo'ob kili'ichtak
          </p>
        </div>
      </div>
    </div>
  );
}